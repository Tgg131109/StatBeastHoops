...
//    @State var ssc = [StatSeriesCompare]()
...
//    var sp : Player {
//        return playerDataManager.sp ?? Player.demoPlayer
//    }
    ...
    var spt : Team {
        if let team = Team.teamData.first(where: { $0.teamID == playerDataManager.sp?.teamID}) {
            return team
        } else {
...
//        return playerDataManager.searchResults
//    }
...
//        NavigationStack {
                ...
                VStack {
                    ...
                    ZStack(alignment: .top) {
                        ...
                        HStack {
                            ...
                            .onChange(of: criteria) {
                                vm.getChartData(criteria: criteria, pIDs: ["\(p1ID)", "\(p2ID)"])
                            }
                        }.padding(.horizontal, 20)
                    }
                    ...
                    ZStack {
                        HStack {
                            if !playerDataManager.leaders.isEmpty {
                    ...
                    List {
                        ForEach(vm.statCompare, id: \.id) { stat in
                            ZStack {
                                HStack {
                                    VStack(alignment: .leading) {
                                        Text(stat.value1)
...
//            .opacity(showOverlay ? 0.3 : 1)
        ...
        .onAppear(perform: {
//            showOverlay = needsOverlay
            
//            if !needsOverlay {
//                showOverlay = needsOverlay
//            }
            
//            if let sel = playerDataManager.sp {
//                sp = sel
//            }
            
            if let pl1 = playerDataManager.p1 {
                p1 = pl1
            } else {
                if !playerDataManager.leaders.isEmpty {
                    p1 = playerDataManager.p1 ?? playerDataManager.leaders[0]
                } else {
                    p1 = Player.demoPlayer
                }
            }
            
            if let pl2 = playerDataManager.p2 {
                p2 = pl2
            } else {
                if !playerDataManager.leaders.isEmpty {
                    p2 = playerDataManager.p2 ?? playerDataManager.leaders[1]
                } else {
    ...
    private var compareOverlay: some View {
        if playerDataManager.needsOverlay {
            VStack {
                Text("Set Matchup").font(.body).bold().padding(.vertical, 20)
                HStack {
                    AsyncImage(url: URL(string: "https://cdn.nba.com/headshots/nba/latest/1040x760/\(sp.playerID).png")) { image in
                        image
                            .resizable()
                            .scaledToFit()
                    } placeholder: {
                    ...
                    Text("\(sp.firstName) \(sp.lastName) selected").bold().italic()
                }.frame(maxWidth: .infinity)
                ...
                HStack {
                    ZStack {
                        VStack(alignment: .leading) {
                            Text(pos == 1 ? "\(playerDataManager.sp?.firstName ?? Player.demoPlayer.firstName)" : "\(p1.firstName)").font(.title).bold()
                            Text(pos == 1 ? "\(playerDataManager.sp?.lastName ?? Player.demoPlayer.lastName)" : "\(p1.lastName)").font(.system(size: 35)).fontWeight(.black).lineLimit(2).minimumScaleFactor(0.8).fixedSize(horizontal: false, vertical: true).multilineTextAlignment(.leading).padding(.top, -30)
                        }.scaleEffect(p1Scale)
                            .animation(.linear(duration: 0.1), value: p1Scale)
                            .frame(maxHeight: 160, alignment: .top)
                            .padding(.horizontal)
                        
                        AsyncImage(url: URL(string: "https://cdn.nba.com/headshots/nba/latest/1040x760/\(pos == 1 ? sp.playerID : p1ID).png")) { image in
                            image
                                .resizable()
                                .scaledToFit()
                    ...
                    ZStack {
                        VStack(alignment: .trailing) {
                            Text(pos == 2 ? "\(sp.firstName)" : "\(p2.firstName)").font(.title).bold()
                            Text(pos == 2 ? "\(sp.lastName)" : "\(p2.lastName)").font(.system(size: 35)).fontWeight(.black).lineLimit(2).minimumScaleFactor(0.8).fixedSize(horizontal: false, vertical: true).multilineTextAlignment(.trailing).padding(.top, -30)
                        }.scaleEffect(p2Scale)
                            .animation(.linear(duration: 0.1), value: p2Scale)
                            .frame(maxHeight: 160, alignment: .top)
                            .padding(.horizontal)
                        
                        AsyncImage(url: URL(string: "https://cdn.nba.com/headshots/nba/latest/1040x760/\(pos == 2 ? (sp.playerID) : p2ID).png")) { image in
                            image
                                .resizable()
                                .scaledToFit()
                ...
                if pos != 0 {
                    Button {
                        if pos == 1 {
                            playerDataManager.p1 = sp
                            p1 = sp
                            p1Scale = 1.0
                            
                        } else {
                            playerDataManager.p2 = sp
                            p2 = sp
                            p2Scale = 1.0
                        }
...
//                        .padding()
                ...
                Button {
                    if !playerDataManager.leaders.isEmpty {
                        Task {
                            await vm.compareStats(p1ID: "\(p1.playerID)", p2ID: "\(p2.playerID)", criteria: criteria)
                        }
                    }
                    
                    withAnimation {
                        playerDataManager.needsOverlay.toggle()
                        playerDataManager.currentDetent = PresentationDetent.large
                    }
            ...
            .onChange(of: playerDataManager.sp?.id) {
                pos = 0
                b1Scale = 1.0
                b2Scale = 1.0
                p2Scale = 1.0
...
//                sp = playerDataManager.sp ?? Player.demoPlayer
            }
        }
    }
    ...
    func gamesPlayed() -> [Double] {
        ...
        if let g = vm.statCompare.first(where: { $0.stat == "GP"}) {
